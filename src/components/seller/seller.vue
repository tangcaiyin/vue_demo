...
<script type='text/ecmascript-6'>
	...
	export default {
		...
		// BScroll 必须是在DOM完全渲染后实例化，
		// 生命周期 mounted() 下，DOM已完全渲染，可以实例化滚动条
		mounted() {
			this.$nextTick(() => {
				this._initScroll();
				this._initPics();
		...
		methods: {
			...
			// better-scroll 要求内容宽高大于外部容器宽高，
			// 实现图片左右滑动时，ul宽度100%，需要动态设置ul宽度才能使用better-scroll，
			_initPics() {
				if (this.seller.pics) {
					let picWidth = 120;
					let margin = 6;
					let width = (picWidth + margin) * this.seller.pics.length - margin;
					// 为ul设置固定宽度，才能使用better-scroll
